...
# Test cases where the infection parameters are varied while network size and density parameters are kept constant
# --------------------------------------------------------------------------
...
beta_values = [0.5,]#[0.5, 0.5, 0.1, 0.8, 0.8]
#alpha:= Rate of infection upon exposure (E->I)
alpha_values = [0.1]#[0.1, 0.9, 0.4, 0.5, 0.5]
#delta:= Rate of recovery (I->R)
delta_values = [0.08]#[0.08, 0.08, 0.1, 0.4, 0.4]
#gamma:= Rate of re-suscptibility (R->S)
gamma_values = [0.5]#[0.5, 0.5, 0.6, 0, 0.9]

# Generate random matrix and save it in a file
...
for i_config in range(len(beta_values)):
    ...
    beta = beta_values[i_config]
    alpha = alpha_values[i_config]
    delta = delta_values[i_config]
    gamma = gamma_values[i_config]
    ...
    file_name = f"TCipResults_N{N}_D{str(density).replace('.', '')}_a{str(alpha).replace('.', '')}_b{str(beta).replace('.', '')}_d{str(delta).replace('.', '')}_g{str(gamma).replace('.', '')}SA"
    with open(f'{file_name}.txt', 'w') as file:
        file.write('[\n')
        for orgNode in total_infected:
            file.write('[' + ','.join(map(str, orgNode))+ '],'+ '\n')

    file_name_top = f"TCipTop_N{N}_D{str(density).replace('.', '')}_a{str(alpha).replace('.', '')}_b{str(beta).replace('.', '')}_d{str(delta).replace('.', '')}_g{str(gamma).replace('.', '')}"
