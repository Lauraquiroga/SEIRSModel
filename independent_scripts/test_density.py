...
# Test cases where Density is varied while network size and the infection parameters are kept constant
# --------------------------------------------------------------------------
...
density_values = [0.2, 0.8] #0.5 es el mismo caso de los parametros
...
for density in density_values:
    ...
    file_name = f"TCdResults_N{N}_D{str(density).replace('.', '')}_a{str(alpha).replace('.', '')}_b{str(beta).replace('.', '')}_d{str(delta).replace('.', '')}_g{str(gamma).replace('.', '')}"
    with open(f'{file_name}.txt', 'w') as file:
        file.write('[\n')
    ...
    file_name_top = f"TCdTop_N{N}_D{str(density).replace('.', '')}_a{str(alpha).replace('.', '')}_b{str(beta).replace('.', '')}_d{str(delta).replace('.', '')}_g{str(gamma).replace('.', '')}"
